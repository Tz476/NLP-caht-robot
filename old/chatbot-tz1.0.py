...
def mission_sequence(player_name):
    ...
    for module in modules:
        ...
        if success:
            ...
            print(f"TZ: ...Multiple failed attempts detected. Initiating backup plan...")
...
def tell_war_story(player_name):
    if not story_state["war_story_parts"]:
        # Generate the full story and split it into three parts
        prompt = (
        ...
        full_story = generate_response(prompt)
        # Split the story into paragraphs
        paragraphs = full_story.strip().split('\n\n')
        # Determine how many paragraphs per part
        total_paragraphs = len(paragraphs)
        part_size = max(1, total_paragraphs // 3)
        # Split into three parts
        story_state["war_story_parts"] = [
            '\n\n'.join(paragraphs[0:part_size]),
            '\n\n'.join(paragraphs[part_size:2*part_size]),
            '\n\n'.join(paragraphs[2*part_size:])
        ]
    # Get the current part
    current_part = story_state["current_story_part"]
    if current_part < len(story_state["war_story_parts"]):
        story_part = story_state["war_story_parts"][current_part]
        print("\nTZ: ")
        lines = story_part.strip().split('\n')
        for line in lines:
            if line.strip() != '':
                print(line.strip())
                time.sleep(2)  # 每行间隔2秒
        story_state["current_story_part"] += 1
    else:
        print("\nTZ: ...I have no more stories to share at this time...")
    time.sleep(2)
